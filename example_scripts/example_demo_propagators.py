"""mPIE reconstruction with different propagator types.

Runs the same simulated dataset through mPIE with Fraunhofer, Fresnel, and ASP
propagators to verify the fused JIT kernel works for each. Prints timing for comparison.

Usage:
    uv run python example_scripts/demo_propagators.py
"""
...
# ============================================================
# 2. Run mPIE with different propagators
# ============================================================
propagators = [
    "Fraunhofer",
    "Fresnel",
...
for prop_name in propagators:
    ...
    monitor = Monitor()
    monitor.figureUpdateFrequency = 10  # disable visualization
    monitor.verboseLevel = "low"

    params = Params()
    params.positionOrder = "random"
    params.propagatorType = prop_name
    params.intensityConstraint = "standard"  # standard, fluctuation, exponential, poisson, proxpoisson
...
plt.close("all")
print(f"\n{'=' * 60}")
print("All propagator tests completed!")
print(f"{'=' * 60}")
