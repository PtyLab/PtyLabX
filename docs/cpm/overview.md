...
Reconstruction     (holds mutable state: object, probe, positions)
    |
    v
Engine.reconstruct()   (iterative phase retrieval: ePIE, mPIE, ...)
    |
    v
...
You choose how to initialize each: `"ones"` or `"circ"` for the object, `"circ"` or `"gaussian"` for the probe.
...
The `Monitor` shows live plots of the reconstruction progress.
...
| **Intensity constraint** | Replacing computed amplitude with measured amplitude while keeping the phase |

## Multi-modal capabilities

PtyLabX supports advanced reconstruction modes by adjusting a few parameters on the `Reconstruction` object:

| Parameter | Default | Description |
|-----------|---------|-------------|
| `npsm` | 1 | Number of incoherent probe modes (partial coherence) |
| `nosm` | 1 | Number of incoherent object modes (object state mixture) |
| `nlambda` | 1 | Number of wavelengths (polychromatic reconstruction) |
| `nslice` | 1 | Number of object slices (multislice for thick samples) |

!!! note
    Polychromatic reconstruction requires a multi-wavelength engine (`ePIE_mw` or `mPIE_mw`) and `spectralDensity` in the experimental data. Multislice reconstruction requires the `e3PIE` engine.
