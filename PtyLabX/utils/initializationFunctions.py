...
def initialProbeOrObject(shape, type_of_init, data, logger: logging.Logger | None = None):
    ...
    if type(type_of_init) is np.ndarray:  # it has already been run
        if logger is not None:
            logger.warning("initialObjectOrProbe was called but the object has already been initialized. Skipping.")
        return type_of_init
    ...
    if type_of_init == "upsampled":
        ...
        upsampled = np.pad(low_res, pad_size, mode="constant", constant_values=0)  # * data.No / data.Np
